...
class PowerFlowData(Dataset):
    """PowerFlowData(Dataset)
    ...
    def __init__(self, root, case=14, split=None, task="train", transform=None, pre_transform=None, pre_filter=None):
        assert len(split) == 3
    ...
    def processed_file_names(self) -> List[str]:
        self.data = torch.load(os.path.join(self.processed_dir, "case"+f"{self.case}"+"_processed_data.pt"))
    ...
    def process(self):
        ...
        edge_features = torch.from_numpy(np.load(self.raw_paths[1]))
        node_features_x = torch.from_numpy(np.load(self.raw_paths[2]))
        node_features_y = torch.from_numpy(np.load(self.raw_paths[3]))
        ...
        data_list = [
            Data(
                x=node_features_x[i],
                y=node_features_y[i],
                edge_index=edge_features[i, :, 0:2].T,
...
class _PowerFlowData(data.Dataset):
    """PowerFlowData

    Arguments:
        tensor -- the tensor of the data
        split -- the split of the data, order by train, val, test
        task -- the task of the data, i.e. train, val, test
    """
    split_order = {
        "train": 0,
        "val": 1,
    ...
    def __init__(self, tensor, split=[.5, .2, .3], task="train"):
        assert task in ["train", "val", "test"]
        super().__init__() # although it is not necessary
        self.tensor = tensor
        self.split = [int(len(tensor) * i) for i in split]
        self.split_tensor = torch.split(self.tensor, self.split, dim=0)[self.split_order[task]]
        self.task = task
        
    def __len__(self):
        return len(self.split_tensor)
    
    def __getitem__(self, index):
        return self.split_tensor[index]
    
    def __repr__(self):
        print(f"PowerFlowData({self.task})")
        print(f"tensor.shape = {self.tensor.shape}")
        print(f"split = {self.split}")
        print(f"split_tensor.shape = {self.split_tensor.shape}")
        
    @classmethod
    def from_numpy(cls, np_array, split=[.5, .2, .3], task="train"):
        return cls(torch.from_numpy(np_array), split, task)

...
def main():
    ...
    # Step 1: Load data
    trainset = PowerFlowData(root='data', case=grid_case,
                             split=[.5, .2, .3], task='train')
    valset = PowerFlowData(root='data', case=grid_case,
    ...
    for epoch in tqdm(range(num_epochs), position=0, leave=True):
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs}: train_loss={train_loss:.4f}, val_loss={val_loss:.4f}, test_loss={test_loss:.4f}")
        if log_to_wandb:

...
feature_names_output = [
    ...
    'Voltage Angle (degrees)',        # --- we care about this
...
if GET_RESULTS:
    ...
    for case in cases:
        ...
        for i, sample in enumerate(testset[:sample_number]):
            ...
            preds.append(result.to(device))
            targets.append(sample.y.to(device))
            masks.append(sample.x[:, 10:].to(device))
            types.append(
                np.argmax(np.array(sample.x[:, :4].to(device)), axis=1))
        ...
        mean = testset.xymean[0].to(device)
        std = testset.xystd[0].to(device)
